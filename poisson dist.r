...
list.files(".secrets/")
gs4_auth(cache = ".secrets", email = "nahom.taye69@gmail.com")

mlbhome<-read_sheet("1Jq4HQzEU4An2AjpRlJNhVIna52hO4CxF_ooa7oOr03U", range= "home")%>%
  mutate(RFper=RF/GP, RAper= RA/GP, OffStr=RFper/(mean(RFper)), DefStr=RAper/(mean(RAper)))%>%
  arrange(desc(RF), RA) %>%
  mutate(Rank=row_number(-RF))%>%
  relocate(Rank,.before = Team)

mlbaway <- read_sheet("1Jq4HQzEU4An2AjpRlJNhVIna52hO4CxF_ooa7oOr03U", range= "away")%>%
  mutate(RFper=RF/GP, RAper= RA/GP, OffStr=RFper/(mean(RFper)), DefStr=RAper/(mean(RAper)))%>%
  arrange(desc(RF), RA) %>%
  mutate(Rank=row_number(-RF))%>%
  relocate(Rank,.before = Team)
...
# Function to calculate lambdas for soccer
calculate_soccer_lambdas <- function(home_team, away_team, home_table, away_table) {
...
# Function to calculate lambdas for baseball
calculate_baseball_lambdas <- function(home_team, away_team, home_table, away_table) {
  # Extract relevant information for the home team
  home_info <- home_table %>% filter(Team == home_team) 
  home_OffStr <- home_info$OffStr
  home_DefStr <- home_info$DefStr
  home_league_avg <- mean(home_table$RFper)
  
  # Extract relevant information for the away team
  away_info <- away_table %>% filter(Team == away_team)
  away_OffStr <- away_info$OffStr
  away_DefStr <- away_info$DefStr
  away_league_avg <- mean(away_table$RFper)
  
  # Calculate lambdas for baseball
  lambda_home <- home_OffStr * away_DefStr * home_league_avg
  lambda_away <- away_OffStr * home_DefStr * away_league_avg
  
  return(list(lambda_home = lambda_home, lambda_away = lambda_away))
}

# Function to perform simulation and return probabilities and goal/run counts
simulate_match <- function(home_team, away_team, home_lambda, away_lambda, sport, simulations = 10000000) {
  if (sport == "soccer") {
    # Simulate goal counts for soccer
    home_team_goals <- rpois(simulations, home_lambda)
    away_team_goals <- rpois(simulations, away_lambda)
    
    # Calculate win, draw, and loss probabilities for soccer
    ...
    n <- 0:6
    home_team_goals_distribution <- round(dpois(n, lambda = home_lambda) * 100,4)
    away_team_goals_distribution <- round(dpois(n, lambda = away_lambda) * 100,4)
    ...
    # Simulate run counts for baseball
    home_team_runs <- rpois(simulations, home_lambda)
    away_team_runs <- rpois(simulations, away_lambda)
    
    # Calculate win, draw, and loss probabilities for baseball
    win_prob <- mean(home_team_runs > away_team_runs) * 100
    OT <- mean(home_team_runs == away_team_runs) * 100
    loss_prob <- mean(home_team_runs < away_team_runs) * 100
    ...
    n <- 0:10
    home_team_runs_distribution <- dpois(n, lambda = home_lambda) * 100
    away_team_runs_distribution <- dpois(n, lambda = away_lambda) * 100
    Total_runs <- data.frame(
      home_team_runs_distribution,
      Runs = n,
      away_team_runs_distribution
    )
    
    colnames(Total_runs)[colnames(Total_runs) == "home_team_runs_distribution"] <- home_team
    colnames(Total_runs)[colnames(Total_runs) == "away_team_runs_distribution"] <- away_team
    ...
    cat("Invalid sport type. Please choose 'soccer' or 'mlb'.\n")
    return(NULL)
  }
}
...
simulate_match_for_user_input <- function() {
  sport_input <- readline("Enter the sport : ")
  
  if (sport_input == "soccer") {
  ...
  } else if (sport_input == "mlb") {
    home_table_input <- mlbhome
    away_table_input <- mlbaway
  } else {
    cat("Invalid sport type. Please choose 'soccer' or 'mlb'.\n")
    return(NULL)
  }
  ...
  if (sport_input == "soccer") {
    lambdas <- calculate_soccer_lambdas(home_team_input, away_team_input, home_table_input, away_table_input)
    probabilities <- simulate_match(home_team_input, away_team_input, lambdas$lambda_home, lambdas$lambda_away, "soccer")
  } else if (sport_input == "mlb") {
    lambdas <- calculate_baseball_lambdas(home_team_input, away_team_input, home_table_input, away_table_input)
    probabilities <- simulate_match(home_team_input, away_team_input, lambdas$lambda_home, lambdas$lambda_away, "mlb")
  }
